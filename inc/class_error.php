...
class errorHandler {
	...
	public $error_types = array( 
		...
		E_USER_NOTICE        => '用户注意',
		E_STRICT             => '运行时注意',
		E_RECOVERABLE_ERROR  => '可捕捉的致命错误',
		MYBB_SQL 			 => 'MyBB SQL 错误', 
		MYBB_TEMPLATE		 => 'MyBB 模板错误',
		MYBB_GENERAL		 => 'MyBB 错误',
		MYBB_NOT_INSTALLED	 => 'MyBB 错误',
		MYBB_NOT_UPGRADED 	 => 'MyBB 错误',
		MYBB_INSTALL_DIR_EXISTS => 'MyBB 错误',
		MYBB_SQL_LOAD_ERROR  => 'MyBB 错误',
		MYBB_CACHE_NO_WRITE  => 'MyBB 错误',
		MYBB_CACHEHANDLER_LOAD_ERROR => 'MyBB 错误',
	);
	...
	function error($type, $message, $file=null, $line=0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					echo "<div class=\"php_warning\">MyBB 内部警告: 一个或多个警告产生。请联系你的系统管理员以获取帮助。</div>"; 
				}
				else
				{
					global $templates;
					
					$warning = "<strong>{$this->error_types[$type]}</strong> [$type] $message - 行: $line - 文件: $file PHP ".PHP_VERSION." (".PHP_OS.")<br />\n";
	...
	function log_error($type, $message, $file, $line)
	{
		...
		if($type == MYBB_SQL)
		{
			$message = "SQL 错误: {$message['error_no']} - {$message['error']}\n查询: {$message['query']}";
		}
		$error_data = "<error>\n";
	...
	function email_error($type, $message, $file, $line)
	{
		...
		$message = "您安装的MyBB {$mybb->settings['bbname']} ({$mybb->settings['bburl']})遇到了一个错误。错误详情包括:\n---\n类型: $type\n文件: $file (行 $line)\n信息\n$message";

		@my_mail($mybb->settings['adminemail'], "MyBB 错误 {$mybb->settings['bbname']}", $message, $mybb->settings['adminemail']);
	}
	...
	function output_error($type, $message, $file, $line)
	{
		...
		{
			$title = "MyBB 内部错误";
			$error_message = "<p>MyBB遇到一个内部错误并且无法继续运行。</p>";
			if($mybb->settings['errortypemedium'] == "both" || $mybb->settings['errortypemedium'] == "error" || defined("IN_INSTALL") || defined("IN_UPGRADE"))
			{
				$error_message .= "<dl>\n";
				$error_message .= "<dt>错误类型:</dt>\n<dd>{$this->error_types[$type]} ($type)</dd>\n";
				$error_message .= "<dt>错误信息:</dt>\n<dd>{$message}</dd>\n";
				if(!empty($file))
				{
					$error_message .= "<dt>位置:</dt><dd>文件: {$file}<br />行: {$line}</dd>\n";
...
<head profile="http://gmpg.org/xfn/11">
	...
	<title>{$mybb->settings['bbname']} - 内部错误</title>
	<style type="text/css">
...
<body>
	<div id="container">
		...
		<div id="content">
			...
			<div id="error">
				{$error_message}
				<p id="footer">Please contact the <a href="http://mybb.com">MyBB Group</a> for support. 请访问<a href="http://www.mybbchina.net">MyBB中文站</a>获取支持.</p>
			</div>
		</div>
	</div>
</body>
...
EOF;
	...
	<div id="mybb_error_content">
		...
		{$error_message}
			<p id="mybb_error_footer">Please contact the <a href="http://mybb.com">MyBB Group</a> for support. 请访问<a href="http://www.mybbchina.net">MyBB中文站</a>获取支持.</p>
		</div>
	</div>
EOF;
		}
	...
	function generate_backtrace()
	{
		if(function_exists("debug_backtrace"))
		{
			...
			$backtrace .= "<thead><tr>\n";
			$backtrace .= "<th style=\"border-bottom: 1px solid #aaa; background: #ccc; padding: 4px; text-align: left; font-size: 11px;\">文件</th>\n";
			$backtrace .= "<th style=\"border-bottom: 1px solid #aaa; background: #ccc; padding: 4px; text-align: left; font-size: 11px;\">行</th>\n";
			$backtrace .= "<th style=\"border-bottom: 1px solid #aaa; background: #ccc; padding: 4px; text-align: left; font-size: 11px;\">函数</th>\n";
			$backtrace .= "</tr></thead>\n<tbody>\n";
