...
else
{
	...
	{	
		...
		{
			$output->print_error("您的用户ID无法验证。这可能是因为恶意的Javascript正试图自动退出。如果您打算登出，请在顶部的菜单点击退出按钮。");
		}
	...
	{	
		...
		else
		{
			...
			if(!$user['uid'])
			{
				$output->print_error("输入的密码无效。如果忘记密码，点击<a href=\"../member.php?action=lostpw\">这里</a>。 否则，回去再试一次。");
			}
		}
	...
	if($mybb->user['uid'] == 0)
	{
		$output->print_header("请登陆", "errormsg", 0, 1);
		
		$output->print_contents('<p>在升级过程开始之前，请输入你的用户名和密码。要执行升级过程，你必须是论坛的有效管理员。</p>
<form action="upgrade.php" method="post">
	<div class="border_wrapper">
		<table class="general" cellspacing="0">
		<thead>
			<tr>
				<th colspan="2" class="first last">登陆</th>
			</tr>
		</thead>
		<tbody>
			<tr class="first">
			...
			<tr class="alt_row last">
				<td class="first">密码:<br /><small>Please note that passwords are case sensitive.</small></td>
	...
	<div id="next_button">
		<input type="submit" class="submit_button" name="submit" value="登陆" />
	...
	{
		$output->print_error("你没有权限运行升级过程。你需要管理员权限才能运行升级程序。<br /><br />如果你想退出，请点击 <a href=\"upgrade.php?action=logout&amp;logoutkey={$mybb->user['logoutkey']}\">这里</a>。然后使用管理员帐号登陆再试一次。");
	}
	...
	elseif($mybb->input['action'] == "doupgrade")
	{
		...
		if($db->table_exists("datacache") && $upgrade_detail['requires_deactivated_plugins'] == 1 && $mybb->input['donewarning'] != "true")
		{
			...
			{
				$output->print_header();
				$lang->plugin_warning = "<input type=\"hidden\" name=\"from\" value=\"".intval($mybb->input['from'])."\" />\n<input type=\"hidden\" name=\"donewarning\" value=\"true\" />\n<div class=\"error\"><strong><span style=\"color: red\">警告:</span></strong> <p>任然有 ".count($plugins['active'])." 个插件处于激活状态。这有时候可能在升级过程中导致未知的问题或者在升级之后导致论坛无法访问。<strong>强烈</strong>推荐你先取消激活这些插件，然后再继续。</p></div> <br />";
...
function upgradethemes()
{
	...
	if($system_upgrade_detail['revert_all_themes'] > 0)
	{
		...
		{
			$output->print_error("请确定 admin 目录正确上传。");
		}
...
function buildsettings()
{
	...
	{
		$output->print_header("重建设置");
		echo "<p><div class=\"error\"><span style=\"color: red; font-weight: bold;\">错误: 无法打开 inc/settings.php</span><h3>再继续升级过程之前，你需要设置 inc/settings.php 的权限为可写。</h3></div></p>";
		$output->print_footer("rebuildsettings");
		exit;
	}
