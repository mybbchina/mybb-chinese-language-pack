...
function upgrade17_dbchanges5()
{
	...
	{
		$output->print_error("请确定你的admin目录已经正确上传。");
	}
...
function upgrade17_dbchanges6()
{
	...
	if($remaining && $ipaddress)
	{
		...
		$startat = $startat+$ipp;
		$contents = "<p><input type=\"hidden\" name=\"ipspage\" value=\"$ipp\" /><input type=\"hidden\" name=\"ipstart\" value=\"$startat\" />完成。点击下一步修复下一组IP。</p>";
	}
	else
	{
		$nextact = "17_dbchanges7";
		$contents = "<p>完成</p><p>所有发帖IP已经成功修复。点击下一步继续。</p>";
	}
	$output->print_contents($contents);
...
function upgrade17_dbchanges7()
{
	...
	if($remaining && $ipaddress)
	{
		$nextact = "17_dbchanges7";
		$startat = $startat+$ipp;
		$contents .= "<p><input type=\"hidden\" name=\"ipspage\" value=\"$ipp\" /><input type=\"hidden\" name=\"ipstart\" value=\"$startat\" />完成。点击下一步修复下一组用户IP。</p>";
	}
	else
	{
		$nextact = "17_redoconfig";
		$contents .= "<p>完成</p><p>所有用户IP已经成功修复。点击下一步继续。</p>";
	}
	$output->print_contents($contents);
...
function upgrade17_redoconfig()
{
	...
	$output->print_header("重写 config.php");
	...
	if(!is_array($config['memcache']))
	{
		...
		if(!$fh)
		{
			echo "<p><span style=\"color: red; font-weight: bold;\">无法打开 inc/config.php</span><br />在升级过程继续之前，你需要设置 inc/config.php 的权限为可写。</p>";
